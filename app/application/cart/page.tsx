...
import { IconTrash, IconArrowRight, IconPackage, IconPlane, IconDeviceFloppy } from "@tabler/icons-react";
...
export default function CartPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<typeof nonTangibleCartItems[0] | null>(null);
            ...
            <Stack gap="md">
              <Group grow>
                <NumberInput
                  ...
                  onChange={(value) => setBookingData({...bookingData, quantity: typeof value === 'number' ? value : 1})}
                  min={1}
                  max={100}
                />
                <TextInput
                  ...
                  onChange={(event) => setBookingData({...bookingData, duration: event.currentTarget.value})}
                />
              </Group>
              ...
              <Group grow>
                <TextInput
                  ...
                  onChange={(event) => setBookingData({...bookingData, startDate: event.currentTarget.value})}
                />
                <TextInput
                  label="End Date"
                  type="date"
                  value={bookingData.endDate}
                  onChange={(event) => setBookingData({...bookingData, endDate: event.currentTarget.value})}
                />
              </Group>

              {selectedItem.category === "Travel" && selectedItem.name.includes("Flight") && (
                <>
                  <Group grow>
                    <TextInput
                      label="From"
                      ...
                      onChange={(event) => setBookingData({...bookingData, from: event.currentTarget.value})}
                    />
                    <TextInput
                      label="To"
                      placeholder="Destination city"
                      value={bookingData.to || ""}
                      onChange={(event) => setBookingData({...bookingData, to: event.currentTarget.value})}
                    />
                  </Group>
                  <Select
                    label="Flight Type"
                    value={bookingData.flightType}
                    onChange={(value) => setBookingData({...bookingData, flightType: value || "Economy"})}
                    data={["Economy", "Business", "First Class"]}
                  />
                </>
              )}

              {selectedItem.category === "Travel" && selectedItem.name.includes("Hotel") && (
                <Group grow>
                  <Select
                    label="Room Type"
                    value={bookingData.roomType}
                    onChange={(value) => setBookingData({...bookingData, roomType: value || "Standard"})}
                    data={["Standard", "Deluxe", "Suite", "Executive"]}
                  />
                  <NumberInput
                    label="Number of Guests"
                    value={bookingData.guests}
                    onChange={(value) => setBookingData({...bookingData, guests: typeof value === 'number' ? value : 1})}
                    min={1}
                    max={10}
                  />
                </Group>
              ...
              {selectedItem.category === "Transport" && selectedItem.name.includes("Car") && (
                <Select
                  label="Car Type"
                  value={bookingData.carType}
                  onChange={(value) => setBookingData({...bookingData, carType: value || "Economy"})}
                  data={["Economy", "Compact", "Mid-size", "Full-size", "SUV", "Luxury"]}
                />
              )}
              ...
              <Textarea
                ...
                onChange={(event) => setBookingData({...bookingData, notes: event.currentTarget.value})}
                rows={3}
              ...
              <Group justify="flex-end" mt="md">
                ...
                <Button leftSection={<IconDeviceFloppy size={16} />} onClick={() => setEditModalOpen(false)}>
                  Save Booking
                </Button>
              </Group>
            </Stack>
        ...
        >
          <Stack gap="md">
            ...
            {cartItemsCart.length > 0 && (
              <>
                <Text fw={600} size="sm">Inventory Items</Text>
                <Stack gap="xs">
                  {cartItemsCart.map((item) => (
                    <Paper key={item.id} p="sm" withBorder>
                      <Group justify="space-between">
                        ...
                        <Text size="sm" fw={600}>KES {(item.price * item.quantity).toLocaleString()}</Text>
                      </Group>
                    </Paper>
                  ))}
                </Stack>
            ...
            {cartItemsCart.length > 0 && nonTangibleCartItems.length > 0 && <Divider />}
            
            {nonTangibleCartItems.length > 0 && (
              <>
                <Group justify="space-between" align="center">
                  <Text fw={600} size="sm">Non-Tangible Services</Text>
                  <Button 
                    size="xs" 
                    variant="light"
                    onClick={() => {
                      ...
                      <Group justify="space-between">
                        <Group>
                          <Checkbox
                            ...
                            onChange={() => {
                              setSelectedService(selectedService === item.id ? "" : item.id);
                            }}
                          />
                          <div>
                            <Text size="sm" fw={500}>{item.name}</Text>
            ...
            <Group justify="flex-end" mt="md">
              <Button variant="outline" onClick={() => setProceedModalOpen(false)}>
                Cancel
              </Button>
              <Button 
                onClick={() => {
